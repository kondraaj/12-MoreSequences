...
def draw_points_on_circle(window, circle, number_of_points, color):
    ...
    for k in range(len(sequence)):
        circle1 = rg.Circle(sequence[k], 10)
...
def run_test_pizza():
    ...
    window = rg.RoseWindow(600, 550, title)
    circle = rg.Circle(rg.Point(305, 220), 175)
    circle.fill_color = 'red'
    pizza(window, circle, 260, 'yellow', 1)
    window.close_on_mouse_click()
...
def run_test_polygon():
    ...
    circle = rg.Circle(rg.Point(275, 230), 200)
    circle.outline_thickness = 2
    circle.fill_color = 'red'
    circle.outline_color = 'black'
    polygon(window, circle, 8, 'purple', 6)
    window.close_on_mouse_click()
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    for k in range(len(sequence)):
        if k == len(sequence) - 1:
            line = rg.Line(sequence[-1], sequence[0])
        else:
            line = rg.Line(sequence[k], sequence[k + 1])
        line.color = color
        line.thickness = thickness
        line.attach_to(window)
    window.render()
